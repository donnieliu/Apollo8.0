...
double DpStCost::GetObstacleCost(const StGraphPoint& st_graph_point) {
  ...
  for (const auto* obstacle : obstacles_) {
    ...
    if (s < s_lower) {
      ...
      if (s + follow_distance_s < s_lower) {  //在安全距离外，返回cost为0
        continue;
      } else {
        auto s_diff = follow_distance_s - s_lower + s; //s_diff = 20-(s_slow-s)
        cost += config_.obstacle_weight() * config_.default_obstacle_cost() *  //obstacle_weight:1;  default_obstacle_cost:1e4
                s_diff * s_diff;
      }
    } else if (s > s_upper) {
...
double DpStCost::GetSpatialPotentialCost(const StGraphPoint& point) {
  return (total_s_ - point.point().s()) * config_.spatial_potential_penalty();  //100
}
...
double DpStCost::GetSpeedCost(const STPoint& first, const STPoint& second,
  ...
  if (speed < max_adc_stop_speed && InKeepClearRange(second.s())) {
    ...
    cost += config_.keep_clear_low_speed_penalty() * unit_t_ *  //keep_clear_low_speed_penalty：10
            config_.default_speed_cost();  //default_speed_cost：1000
  }

  double det_speed = (speed - speed_limit) / speed_limit;
  if (det_speed > 0) {
    cost += config_.exceed_speed_penalty() * config_.default_speed_cost() *  //exceed_speed_penalty：1000
            (det_speed * det_speed) * unit_t_;
  } else if (det_speed < 0) {
    cost += config_.low_speed_penalty() * config_.default_speed_cost() *  //low_speed_penalty：10
            -det_speed * unit_t_;
  }

  if (FLAGS_enable_dp_reference_speed) {  //true
    double diff_speed = speed - cruise_speed;
    cost += config_.reference_speed_penalty() * config_.default_speed_cost() * //reference_speed_penalty：10
...
double DpStCost::GetAccelCost(const double accel) {
  ...
  const size_t accel_key = static_cast<size_t>(accel / kEpsilon + 0.5 + kShift); //accel/0.1+0.5+100
  ...
  if (accel_cost_.at(accel_key) < 0.0) {
    ...
    double max_dec = config_.max_deceleration(); //-6
...
double DpStCost::JerkCost(const double jerk) {
  ...
  if (jerk_cost_.at(jerk_key) < 0.0) {
    ...
    if (jerk > 0) {
      cost = config_.positive_jerk_coeff() * jerk_sq * unit_t_;  //1
    } else {
      cost = config_.negative_jerk_coeff() * jerk_sq * unit_t_;  //1
    }

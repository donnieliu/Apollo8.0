...
GriddedPathTimeGraph::GriddedPathTimeGraph(
  ...
  unit_t_ = gridded_path_time_graph_config_.unit_t();  //1s
  total_length_s_ = st_graph_data_.path_length();
  dense_unit_s_ = gridded_path_time_graph_config_.dense_unit_s(); //0.25
  sparse_unit_s_ = gridded_path_time_graph_config_.sparse_unit_s();  //1s
  dense_dimension_s_ = gridded_path_time_graph_config_.dense_dimension_s();  //21
  // Safety approach preventing unreachable acceleration/deceleration
  max_acceleration_ =
      std::min(std::abs(vehicle_param_.max_acceleration()),  //2
...
void GriddedPathTimeGraph::CalculateCostAt(
  ...
  if (c == 1) {
    ...
    if (init_point_.v() + acc * unit_t_ < -kDoubleEpsilon &&
        cost_cr.point().s() > min_s_consider_speed) { //这个什么道理？？，这不很容易就超出这个距离吗
      return;
    }
  ...
  const double pre_lowest_s =
      cost_cr.point().s() -
      FLAGS_planning_upper_speed_limit * (1 + kSpeedRangeBuffer) * unit_t_;  //FLAGS_planning_upper_speed_limit:31.3
...
Status GriddedPathTimeGraph::RetrieveSpeedProfile(SpeedData* const speed_data) {
  ...
  //这么做的目的就是，如果在7s内没到达终点，那么min_cost点一定在最后一列；
  //如果在7s内到达终点，那么min_cost点一定在最后一行的某个列点，在那列就看哪列的cost最小；

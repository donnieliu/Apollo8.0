...
common::Status PiecewiseJerkPathOptimizer::Process(
  ...
  for (const auto& path_boundary : path_boundaries) {
    ...
    //计算横向运动学约束，参考https://zhuanlan.zhihu.com/p/481835121?utm_id=0
    const auto& veh_param =
        common::VehicleConfigHelper::GetConfig().vehicle_param();
    const double lat_acc_bound =
        std::tan(veh_param.max_steer_angle() / veh_param.steer_ratio()) //用曲率近似，dl比较小的情况下
...
bool PiecewiseJerkPathOptimizer::OptimizePath(
  ...
  piecewise_jerk_problem.set_dx_bounds(-FLAGS_lateral_derivative_bound_default, //2.0
  ...
  const double max_yaw_rate =
      veh_param.max_steer_angle_rate() / veh_param.steer_ratio() / 2.0; //这个地方为什么除以2？？
...
FrenetFramePath PiecewiseJerkPathOptimizer::ToPiecewiseJerkPath(
  ...
  while (accumulated_s < piecewise_jerk_traj.ParamLength()) {
    ...
    accumulated_s += FLAGS_trajectory_space_resolution;  //1m
  }

  return FrenetFramePath(std::move(frenet_frame_path));
}

double PiecewiseJerkPathOptimizer::EstimateJerkBoundary(  //参见https://zhuanlan.zhihu.com/p/481835121?utm_id=0

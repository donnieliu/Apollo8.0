...
apollo::common::Status RuleBasedStopDecider::Process(
  ...
  if (FLAGS_enable_lane_change_urgency_checking) {  //false
...
void RuleBasedStopDecider::AddPathEndStop(
  ...
  if (!reference_line_info->path_data().path_label().empty() &&
      reference_line_info->path_data().frenet_frame_path().back().s() -
          ...
          FLAGS_short_path_length_threshold) {    //20
...
void RuleBasedStopDecider::StopOnSidePass(
  ...
  if (!check_clear &&
    ...
    if (!LaneChangeDecider::IsPerceptionBlocked( //如果障碍物没有阻塞，直接return
            *reference_line_info,
        ...
        LaneChangeDecider::IsClearToChangeLane(reference_line_info)) { //注意，这个地方可不是换道，虽然调用的换道LaneChangeDecider的函数
      return;
  ...
  for (const auto &point_guide : path_point_decision_guide) {
    if (last_path_point_type == PathData::PathPointType::IN_LANE &&
        std::get<1>(point_guide) ==
            PathData::PathPointType::OUT_ON_REVERSE_LANE) { //刚进入借道的相反的车道
...
bool RuleBasedStopDecider::CheckADCStop(
  ...
  if (adc_speed > rule_based_stop_decider_config_.max_adc_stop_speed()) { //0.5
  ...
  if (distance_stop_line_to_adc_front_edge >  //停的太远也不行
      rule_based_stop_decider_config_.max_valid_stop_distance()) {  //1m

...
//PathBoundsDecider计算path上的可行驶区域边界，其输出是对总线s等间距采样，纵向s对应的左右边界，以此来作为下一步路径搜索的边界约束；与其他task一样，PathBoundsDecider的主要逻辑在process函数中
//在process（）方法分四种情况对PathBoundDecider进行计算，按照处理顺序分别是fallback,pullover,lanechange,regular,不同的boundary对应不同的应用场景，其中fallback
//对应的path bound一定会生成，其余3个只有一个被激活，即按照顺序一旦有有效的boundary生成，就结束该task。
Status PathBoundsDecider::Process(
  ...
  //判断是否满足lane_change场景
  //首先FLAGS_enable_smarter_lane_change要置位
  //通过判断当前reference_line是否为目标车道，来进行判断
  // If it's a lane-change reference-line, generate lane-change path boundary.
  if (FLAGS_enable_smarter_lane_change &&   //false
      reference_line_info->IsChangeLanePath()) {
    PathBound lanechange_path_bound;
    //当前reference_line如果是目标path，则计算lanechange_path_bound
    //首先根据借道与否，用道路宽度来生成基本的path bound
    //然后遍历path上的每个点，并且判断这个点上的障碍物，利用障碍物的边界来修正path bound
    //如果目标在左边将left_bound设置为右边界
    //如果目标在右边，则将right_bound设置为目标的左边界
  ...
  for (const auto& lane_borrow_info : lane_borrow_info_list) {
    ...
    // disable this change when not extending lane bounds to include adc
    if (config_.path_bounds_decider_config() //false
            .is_extend_lane_bounds_to_include_adc()) {
...
//在process（）的过程中，首先会初始化bounds，用规划起始点自车道的lane width去初始化path boundry，如果无法获取规划起始点的道路宽度，则用默认值5m来初始化。
//path_bound由一系列采样点组成，数据结构如下，这组数据里共有0-199个200个采样点，每个点的间隔是0.5m；每个点由3个变量组成，分别是根据参考线建立的s-l坐标系的s坐标，右边界的l取值，左边界的l取值
void PathBoundsDecider::InitPathBoundsDecider(
  ...
  adc_frenet_ld_ = adc_sl_info.second[1] * adc_frenet_sd_; //l`（横向速度）
  double offset_to_map = 0.0;
  reference_line.GetOffsetToMap(adc_frenet_s_, &offset_to_map);   //offset_to_map:基于参考线的，因此需要考虑参考线相对map的偏移量offset_to_map
  ...
  if (!reference_line.GetLaneWidth(adc_frenet_s_, &lane_left_width,
    ...
    adc_lane_width_ = kDefaultLaneWidth; //5M
  } else {
...
Status PathBoundsDecider::GenerateRegularPathBound(
  ...
  while (!blocking_obstacle_id->empty() &&
         ...
         counter < kNumExtraTailBoundPoint) {  //20
    path_bound->push_back(temp_path_bound[path_bound->size()]);  //增加一些轨迹点，避免0轨迹点，因为障碍物如果完全阻挡住，轨迹点会是0
    counter++;
  }
  // PathBoundsDebugString(*path_bound);
...
Status PathBoundsDecider::GenerateFallbackPathBound(
  ...
  // 2. Decide a rough boundary based on lane info and ADC's position
  std::string dummy_borrow_lane_type;//这里也没考虑借道
  if (!GetBoundaryFromLanesAndADC(reference_line_info,
...
bool PathBoundsDecider::GetBoundaryFromRoads(
  ...
  TrimPathBounds(path_blocked_idx, path_bound); //如果边界计算错误，那么就把错误的边界点之后的边界删除；
  return true;
}
...
bool PathBoundsDecider::GetBoundaryFromLanesAndADC(
    ...
    bool is_fallback_lanechange) {  //true
  // Sanity checks.
  CHECK_NOTNULL(path_bound);
  ACHECK(!path_bound->empty());
  const ReferenceLine& reference_line = reference_line_info.reference_line();
  ...
  for (size_t i = 0; i < path_bound->size(); ++i) {
    ...
    double curr_lane_left_width = 0.0;
    double curr_lane_right_width = 0.0;
    double offset_to_lane_center = 0.0;  //offset_to_lane_center:由于车道中心线和参考线不一定重合的，所以以参考线为基准，左右边界的宽度会做调整
    if (!reference_line.GetLaneWidth(curr_s, &curr_lane_left_width,
                                     &curr_lane_right_width)) {
      AWARN << "Failed to get lane width at s = " << curr_s;
      curr_lane_left_width = past_lane_left_width;
      ...
      reference_line.GetOffsetToMap(curr_s, &offset_to_lane_center);
      curr_lane_left_width += offset_to_lane_center; //往一边偏，正的是参考线往右边偏，负的往左边偏，所以是左加右减去
      curr_lane_right_width -= offset_to_lane_center;
      past_lane_left_width = curr_lane_left_width;
      past_lane_right_width = curr_lane_right_width;
    }
    ...
    // 2. Get the neighbor lane widths at the current point.
    double curr_neighbor_lane_width = 0.0;
    if (CheckLaneBoundaryType(reference_line_info, curr_s, lane_borrow_info)) {   //这里如果考虑换道切换道方向的车道线为实线，那么边界就不考虑换道了
      hdmap::Id neighbor_lane_id;
      if (lane_borrow_info == LaneBorrowInfo::LEFT_BORROW) {
        // Borrowing left neighbor lane.
        if (reference_line_info.GetNeighborLaneInfo(
                ReferenceLineInfo::LaneType::LeftForward, curr_s,  //向左侧同向顺行车道借道
                &neighbor_lane_id, &curr_neighbor_lane_width)) {
          ADEBUG << "Borrow left forward neighbor lane.";
        } else if (reference_line_info.GetNeighborLaneInfo(
                       ReferenceLineInfo::LaneType::LeftReverse, curr_s,  //向左侧反向逆行车道借道
                       &neighbor_lane_id, &curr_neighbor_lane_width)) {
          borrowing_reverse_lane = true;
          ADEBUG << "Borrow left reverse neighbor lane.";
        } else {
        ...
        // Borrowing right neighbor lane.
        if (reference_line_info.GetNeighborLaneInfo(
                ReferenceLineInfo::LaneType::RightForward, curr_s,    //向右侧同向顺行车道借道
                &neighbor_lane_id, &curr_neighbor_lane_width)) {
          ADEBUG << "Borrow right forward neighbor lane.";
        } else if (reference_line_info.GetNeighborLaneInfo(
                       ReferenceLineInfo::LaneType::RightReverse, curr_s,  //向右侧反向逆行车道借道
                       &neighbor_lane_id, &curr_neighbor_lane_width)) {
          borrowing_reverse_lane = true;
          ADEBUG << "Borrow right reverse neighbor lane.";
        } else {
    ...
    double ADC_speed_buffer = (adc_frenet_ld_ > 0 ? 1.0 : -1.0) *  //横向距离buffer
                              adc_frenet_ld_ * adc_frenet_ld_ /
                              kMaxLateralAccelerations / 2.0;
    ...
    if (config_.path_bounds_decider_config()
      ...
      double curr_right_bound_adc =
          ...
          GetBufferBetweenADCCenterAndEdge() - ADC_buffer;
      curr_right_bound =
          std::fmin(curr_right_bound_lane, curr_right_bound_adc) - //考虑借道跟考虑自车信息生成的边界对比，右取小左取大
          offset_to_map;
    } else {
      curr_left_bound = curr_left_bound_lane - offset_to_map; //这个地方都是左右都减去地图偏移，之前有个地方是左边加上，右边减去，为什么如此混乱
    ...
    // 4. Update the boundary.
    if (!UpdatePathBoundaryWithBuffer(i, curr_left_bound, curr_right_bound, //这个的作用是如果左边有lane的话，范围要大半车宽，不然在这个函数之前加的半车宽要被减掉。
...
void PathBoundsDecider::GetBoundaryFromLaneChangeForbiddenZone(
  ...
  auto* lane_change_status = injector_->planning_context()
                                 ->mutable_planning_status()
                                 ->mutable_change_lane();
  if (lane_change_status->is_clear_to_change_lane()) {  //安全变道为true,这个时候不需要lane_change的起点，直接变道；
  ...
  if (lane_change_status->exist_lane_change_start_position()) {
    ...
    lane_change_start_s = FLAGS_lane_change_prepare_length + adc_frenet_s_; //80m；  // 设置为adc前方一段距离为变道起始点

    // Update the decided lane_change_start_s into planning-context. // 更新变道起始点的信息
  ...
  for (size_t i = 0; i < path_bound->size(); ++i) {
    ...
    std::get<1>((*path_bound)[i]) =   //收缩边界区域，避免进入禁行区域
...
bool PathBoundsDecider::GetBoundaryFromStaticObstacles(
  ...
  size_t obs_idx = 0;
  int path_blocked_idx = -1;
  std::multiset<double, std::greater<double>> right_bounds;  //由于大到小
  ...
  for (size_t i = 1; i < path_boundaries->size(); ++i) {
    ...
    if (obs_idx < sorted_obstacles.size() &&
      ...
      while (obs_idx < sorted_obstacles.size() &&
        ...
        if (std::get<0>(curr_obstacle) == 1) {  //起始点
          ...
          if (curr_obstacle_l_min + curr_obstacle_l_max < center_line * 2) {
            ...
            obs_id_to_direction[curr_obstacle_id] = false; //左边false右边true
      ...
      std::get<1>((*path_boundaries)[i]) =
          std::fmax(std::get<1>((*path_boundaries)[i]),
                    *right_bounds.begin() + GetBufferBetweenADCCenterAndEdge()); //为什么要引入这个半车宽？？
      std::get<2>((*path_boundaries)[i]) =
...
std::vector<ObstacleEdge> PathBoundsDecider::SortObstaclesForSweepLine(
  ...
  for (const auto* obstacle : indexed_obstacles.Items()) {
    ...
    sorted_obstacles.emplace_back(
        1, obstacle_sl.start_s() - FLAGS_obstacle_lon_start_buffer,  //3m
        obstacle_sl.start_l() - FLAGS_obstacle_lat_buffer,  //0.4
        obstacle_sl.end_l() + FLAGS_obstacle_lat_buffer, obstacle->Id());
    sorted_obstacles.emplace_back(
        0, obstacle_sl.end_s() + FLAGS_obstacle_lon_end_buffer,   //2m
...
bool PathBoundsDecider::UpdatePathBoundaryWithBuffer(
  ...
  double left_adc_buffer_coeff =
      (is_left_lane_bound  //默认为false
           ? config_.path_bounds_decider_config().adc_buffer_coeff()  //1
           : default_adc_buffer_coeff);
  double right_adc_buffer_coeff =
      (is_right_lane_bound  //默认为false
...
bool PathBoundsDecider::CheckLaneBoundaryType(
  ...
  const auto waypoint = ref_point.lane_waypoints().front();  //这里的waypoint到底是什么？？

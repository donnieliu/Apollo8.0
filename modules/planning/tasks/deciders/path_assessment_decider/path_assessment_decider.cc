...
Status PathAssessmentDecider::Process(
  ...
  if (FLAGS_enable_skip_path_tasks && reference_line_info->path_reusable()) { //false
    return Status::OK();
  }
  ...
  for (size_t i = 0; i != valid_path_data.size(); ++i) {
    ...
    if (curr_path_data.path_label().find("self") != std::string::npos) {
      ...
      blocking_obstacle_on_selflane =  //为什么只找self，不去关注借道，后边继续看？？
  ...
  if (valid_path_data.front().path_label().find("fallback") != //若路径不是fallback路径的话，nurge的buffer加大
      std::string::npos) {
    FLAGS_static_obstacle_nudge_l_buffer = 0.8;  //默认为0.3
  }
  *(reference_line_info->mutable_path_data()) = valid_path_data.front(); //存入path_data，ReferenceLineInfo就不弄个set接口吗？？
  reference_line_info->SetBlockingObstacle(  //在这个地方把path的obstacle_id设置成reference_line_info的id
  ...
  if (reference_line_info->GetBlockingObstacle() != nullptr) {
    int front_static_obstacle_cycle_counter =
        mutable_path_decider_status->front_static_obstacle_cycle_counter(); //这里是哪里设置的？？需要继续看，弄了半天就是这里设置的，默认的初始值为0，在-10~10中变化，每次递增（减）1
  ...
  if (mutable_path_decider_status->is_in_path_lane_borrow_scenario()) {
    ...
    if (right_borrow) {
      //其实在path_lane_borrow_decider中也有add这个的函数，只不过这里重新add的条件（right_borrow）多了一个 reference_line_info->path_data().path_label().find("left")，即生成换道路径成功了才会是真正的换道状态
...
bool ComparePathData(const PathData& lhs, const PathData& rhs,
  ...
  static constexpr double kSelfPathLengthComparisonTolerance = 15.0;  //如果两个路相差大于15的话，选择长的那个（换道），如果不大于15，那么选择直的车道；
  ...
  if (std::abs(lhs_on_reverse - rhs_on_reverse) > 6) {  //如果两条选择逆向借道的路径长度差不多，选择左侧的，如果有一定差距（6个点以上），选择短的，但两条路径怎么可能都借道呢？？
  ...
  if ((lhs.path_label().find("left") != std::string::npos &&
    ...
    if (blocking_obstacle) {
      ...
      ADEBUG << "obstacle[" << blocking_obstacle->Id() << "] l[" << obstacle_l
             << "]";
      return (obstacle_l > 0.0 //大于0是位于左侧，那么选择向右避的路径
                  ? (lhs.path_label().find("right") != std::string::npos)
  ...
  // then select the one that returns to in-lane earlier. //如果长度相同，而且都是前向的，那么选择更早回到主路上来的路径
  ...
  for (const auto& frenet_path_point : frenet_path) {
    ...
    if (reference_line_info.reference_line().GetRoadWidth(
      ...
      if (frenet_path_point.l() > road_left_width + kOffRoadThreshold ||
          frenet_path_point.l() < -road_right_wid - kOffRoadThreshold) {
...
bool PathAssessmentDecider::IsCollidingWithStaticObstacles(
  ...
  for (size_t i = 0; i < path_data.discretized_path().size(); ++i) { //为什么要留出一块路径来？？
    if (path_data.frenet_frame_path().back().s() -
            path_data.frenet_frame_path()[i].s() <
        (kNumExtraTailBoundPoint + 1) * kPathBoundsDeciderResolution) { //(20+1)*0.5 == 10.25
      break;
    }
    ...
    for (const auto& corner_point : ABCDpoints) {
      ...
      for (const auto& obstacle_polygon : obstacle_polygons) {
        if (obstacle_polygon.IsPointIn(curr_point)) {  //有时间具体看一下IsPointIn？？
...
bool PathAssessmentDecider::IsStopOnReverseNeighborLane(
  ...
  for (const auto& stop_point_sl : all_stop_point_sl) {
    if (stop_point_sl.s() - adc_end_s < kLookForwardBuffer) { //这个为什么continue？？
      continue;
    }
...
void PathAssessmentDecider::SetPathPointType(
  ...
  const double ego_back_to_center =
      vehicle_config.vehicle_param().back_edge_to_center(); //后边缘道后轴
  ...
  for (size_t i = 0; i < discrete_path.size(); ++i) {
    ...
    if (reference_line_info.reference_line().GetLaneWidth(   //这里为什么根据s可以直接获取相邻车道的宽度（换道），可能是因为是获取参考线所在的车道宽度，如果获取参考线相邻车道宽度也是比较麻烦？？
      ...
      double in_and_out_lane_hysteresis_buffer = //如果轨迹点不在借的路上，那么判断左右借道会给一定的buffer
          is_prev_point_out_lane ? back_to_inlane_extra_buffer : 0.0;

      // Check for lane-change and lane-borrow differently:
      if (is_lane_change_path) { //true
        // For lane-change path, only transitioning part is labeled as
        // out-of-lane.
        if (ego_sl_boundary.start_l() > lane_left_width ||    //  右变道满足，注意条件是或
            ego_sl_boundary.end_l() < -lane_right_width) {    //左变道满足
        ...
        if (ego_sl_boundary.end_l() >
                lane_left_width + in_and_out_lane_hysteresis_buffer ||   //向左借道
            ego_sl_boundary.start_l() <
                -lane_right_width - in_and_out_lane_hysteresis_buffer) {  //向右借道
          ...
          if (!is_prev_point_out_lane) {
            if (ego_sl_boundary.end_l() >
                ...
                ego_sl_boundary.start_l() <
                    -lane_right_width - back_to_inlane_extra_buffer) {
              is_prev_point_out_lane = true;  //这个状态是表明点在借的道上
            }
          }
        } else {
...
int ContainsOutOnReverseLane(  //统计在逆向车道的轨迹点

  ...
  if (trajectory.trajectory_point().empty()) {
    ...
    for (const auto& curr_point_on_path : path_points) {
      ...
      if (CheckOverlap(curr_point_on_path, obs_box, l_buffer)) {
        ...
        double low_s =
            std::fmax(0.0, curr_point_on_path.s() + backward_distance); //由于障碍车没有轨迹，这里采用估算的办法，估计放大了障碍车占据的范围
    ...
    if (path_points.size() > 2 * default_num_point) {  //只取100个点
    ...
    for (int i = 0; i < trajectory.trajectory_point_size(); ++i) {
      ...
      auto path_len =
          std::min(FLAGS_max_trajectory_len, discretized_path.Length());  //max_trajectory_len：1000
        ...
        if (CheckOverlap(curr_adc_path_point, obs_box, l_buffer)) {
          ...
          if (find_high && find_low) {
            lower_points->emplace_back(
                low_s - speed_bounds_config_.point_extension(),  //0

...
Status SpeedBoundsDecider::Process(
  ...
  //遍历path_decision决策之后的每个目标，并且读取他们的st boundaries中
  ...
  const double min_s_on_st_boundaries = SetSpeedFallbackDistance(path_decision); //返回ST图左下点的最小s

  // 2. Create speed limit along path
  //在生成speed limit时主要调动speed_limit_decider的GetSpeedLimits函数，这个函数的实现过程较为复杂，之后会详解。限速信息存入到speed_limit变量中，数据结构如下，共100个点

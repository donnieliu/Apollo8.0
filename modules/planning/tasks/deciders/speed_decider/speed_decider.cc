...
//GetSTLocation获取障碍物在st图上与自车路径的位置关系
...
bool SpeedDecider::IsFollowTooClose(const Obstacle& obstacle) const {
  ...
  const double distance =
      obstacle.path_st_boundary().min_s() - FLAGS_min_stop_distance_obstacle; //6M
  ...
  return distance < distance_numerator / distance_denominator;  //V2/(2*a)
}
//主要的决策函数SpeedDecider::MakeObjectDecision（）
//1，对每个目标进行遍历，分贝对每个目标进行决策；
...
Status SpeedDecider::MakeObjectDecision(
  ...
  for (const auto* obstacle : path_decision->obstacles().Items()) {
    ...
    if (obstacle->HasLongitudinalDecision()) { //如果有纵向决策，难道是期盼着没有横向决策？？
      AppendIgnoreDecision(mutable_obstacle);
      continue;
    }

    // for Virtual obstacle, skip if center point NOT "on lane"
    //对于虚拟目标Virtual obstacle，如果不在referenceline的车道上，则跳过；
    ...
    //如果GetSTLocation==BELOW
    //1,根据boundary_type是否为KEEP_CLEAR判断是否要stop_decision
    //2,判断是否IsFollowTooClose需要stop_decision还是保持follow_decision
    //3，上述都不满足，判断是否需要yield_decision
    switch (location) {
      case BELOW:
        if (boundary.boundary_type() == STBoundary::BoundaryType::KEEP_CLEAR) {
          ObjectDecisionType stop_decision;
        ...
        break;
        //如果GetSTLocation==ABOVE
        //1,根据boundary_type是否为KEEP_CLEAR判断是否要ignore；
        //2，判断是否需要overtake_dicision
      case ABOVE:
        if (boundary.boundary_type() == STBoundary::BoundaryType::KEEP_CLEAR) {
        ...
        break;
      //如果GetSTLocation==CROSS
      //输出stop_decision,并打印信息
      case CROSS:
...
bool SpeedDecider::CreateFollowDecision(
  ...
  const double reference_s =
      adc_sl_boundary_.end_s() + boundary.min_s() + follow_distance_s; //这个怎么算的
  const double main_stop_s =
      reference_line_info_->path_decision()->stop_reference_line_s();
...
bool SpeedDecider::CheckIsFollow(const Obstacle& obstacle,
  ...
  if (obstacle_l_distance > FLAGS_follow_min_obs_lateral_distance) { //2.5
    return false;
  }
  ...
  if (boundary.max_t() - boundary.min_t() < FLAGS_follow_min_time_sec) { //2S
    return false;
  }

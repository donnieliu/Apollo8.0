...
//LaneFollowStage的Process（）函数执行主要的规划逻辑，在函数内部会对变道的效率进行判断从而选择是否按照变道进行规划，或者保持本车道运行
//下面代码中，当变道时针对目标车道进行规划，如果规划成功后，还需要判断目标车道的变道cost，如果cost太高，那么就会舍去掉这条目标车道的reference_line，
...
//遍历所有的参考线，直到找到可用来规划的参考线后退出
  for (auto& reference_line_info : *frame->mutable_reference_line_info()) {
    ...
    //判断结果是否OK
    if (cur_status.ok()) {
      if (reference_line_info.IsChangeLanePath()) {  //如果发生lanechange，判断reference_line的cost
        ...
        } else {   //如果没有lanechange，stage执行结果为OK，则has_drivable_reference_line置位TRUE
          LaneChangeDecider::UpdatePreparationDistance(
  ...
  return has_drivable_reference_line ? StageStatus::RUNNING  //根据has_drivable_reference_line标志位的结果，返回stage执行的结果
                                     : StageStatus::ERROR;
}
...
Status LaneFollowStage::PlanOnReferenceLine(
    const TrajectoryPoint& planning_start_point, Frame* frame,
    ReferenceLineInfo* reference_line_info) {
       //判断是否有变道意图，如果有则计算cost
  if (!reference_line_info->IsChangeLanePath()) {
    reference_line_info->AddCost(kStraightForwardLineCost);    //10
  }
  ...
  if (!ret.ok()) {  //如果task执行失败，则使用备用的规划轨迹

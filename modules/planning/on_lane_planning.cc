...
Status OnLanePlanning::Init(const PlanningConfig& config) {
  ...
  PlanningBase::Init(config_);  //这个最后调用的是planning_context（）的Init（），实际是空函数。

  planner_dispatcher_->Init();  //这个Init其实就是注册所有的规划器，把各个规划器都放到map里边去。

  ACHECK(apollo::cyber::common::GetProtoFromFile(
      FLAGS_traffic_rule_config_filename, &traffic_rule_configs_))  //FLAGS_traffic_rule_config_filename： /apollo/modules/planning/conf/traffic_rule_config.pb.txt
  ...
  // dispatch planner
  planner_ = planner_dispatcher_->DispatchPlanner(config_, injector_);  //在OnLanePlanning类初始化的时候就定义planner_dispatcher_为OnLanePlannerDispatcher类型的对象。
  if (!planner_) { //通过config里配置的plannertype返回的planner
    return Status(
...
Status OnLanePlanning::InitFrame(const uint32_t sequence_num,
  ...
  if (!reference_line_provider_->GetReferenceLines(&reference_lines,  //获取参考线
                                                   &segments)) {
...
void OnLanePlanning::GenerateStopTrajectory(ADCTrajectory* ptr_trajectory_pb) {
  ptr_trajectory_pb->clear_trajectory_point(); //注意protobuf的用法，clear_，清除数组内容
  ...
  tp.set_v(0.0);
  tp.set_a(0.0);
  for (double t = 0.0; t < max_t; t += unit_t) {  //产生3s的路，100ms的间隔
    tp.set_relative_time(t);
    auto next_point = ptr_trajectory_pb->add_trajectory_point();  //注意add_是protobuf的用法，创建一个对象，返回指向这个对象的指针
    next_point->CopyFrom(tp); //copyfrom 是protobuf的用法
  }
}
...
void OnLanePlanning::RunOnce(const LocalView& local_view,
  ...
  const double start_timestamp = Clock::NowInSeconds();  //start_timestamp和start_system_timestamp区别
  ...
  Status status = injector_->vehicle_state()->Update(           //将底盘和定位赋给车辆状态
      *local_view_.localization_estimate, *local_view_.chassis);

  VehicleState vehicle_state = injector_->vehicle_state()->vehicle_state();  //把车辆状态返回到本类
  ...
  if (!status.ok() || !util::IsVehicleStateValid(vehicle_state)) {  //检查车辆状态是否合法
    ...
    FillPlanningPb(start_timestamp, ptr_trajectory_pb);   //主要是时间戳的赋予  
  ...
  reference_line_provider_->UpdateVehicleState(vehicle_state);  //把刚刚从底盘和定位拿到数据更到车辆状态的数据更新，更新到参考线提供类里边的车辆状态。
  ...
  std::vector<TrajectoryPoint> stitching_trajectory =   //把返回的stitching_trajectory的最后一个点作为规划的起点
      TrajectoryStitcher::ComputeStitchingTrajectory(
          vehicle_state, start_timestamp, planning_cycle_time,
          FLAGS_trajectory_stitching_preserved_length, true,       //FLAGS_trajectory_stitching_preserved_length:20
          last_publishable_trajectory_.get(), &replan_reason);

  injector_->ego_info()->Update(stitching_trajectory.back(), vehicle_state);    //设置起点，计算bounding_box
  const uint32_t frame_num = static_cast<uint32_t>(seq_num_++);   //设置sequence
  status = InitFrame(frame_num, stitching_trajectory.back(), vehicle_state);

  if (status.ok()) {
    injector_->ego_info()->CalculateFrontObstacleClearDistance(  //计算距离本车最近的障碍物
        frame_->obstacles());
  }

  if (FLAGS_enable_record_debug) { //默认为true
  ...
  if (!status.ok()) {
    ...
    if (FLAGS_publish_estop) {  //默认为false
...
bool OnLanePlanning::CheckPlanningConfig(const PlanningConfig& config) {
  if (!config.has_standard_planning_config()) {    //has_xxx()是protobuf2的用法，用来检测这个字段是否设值了，一般这个字段发送端为optional
    return false;
  }
  if (!config.standard_planning_config().has_planner_public_road_config()) {   //这个字段的结构体为空，但配置文件同样设成空，因此has_xxx()返回TRUE。
    return false;
  }
...
VehicleState OnLanePlanning::AlignTimeStamp(const VehicleState& vehicle_state,
  ...
  auto future_xy = injector_->vehicle_state()->EstimateFuturePosition(
      curr_timestamp - vehicle_state.timestamp());   //这个是车辆状态时间戳比当前规划的时间戳晚的时间

...
double Obstacle::MinRadiusStopDistance(
  ...
  double stop_distance =
      std::sqrt(std::fabs(min_turn_radius * min_turn_radius -  //为什么要求差呢，求出来的比min_turn_radius小一点
                          (min_turn_radius - lateral_diff) *
                              (min_turn_radius - lateral_diff))) +
      stop_distance_buffer;
  stop_distance -= vehicle_param.front_edge_to_center();
  stop_distance = std::min(stop_distance, FLAGS_max_stop_distance_obstacle);  //10m
  stop_distance = std::max(stop_distance, FLAGS_min_stop_distance_obstacle);  //6m

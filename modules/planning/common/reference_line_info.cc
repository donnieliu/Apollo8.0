...
bool ReferenceLineInfo::GetNeighborLaneInfo(
  ...
  auto ptr_neighbor_lane =
      hdmap::HDMapUtil::BaseMapPtr()->GetLaneById(*ptr_lane_id);  //这个地方的API不懂BaseMapPtr
  ...
  if (!ptr_neighbor_lane->GetProjection({ref_point.x(), ref_point.y()}, //这个neighbor_s和s不一样？？，但看之前的资料的确可能不一样，注意
...
bool ReferenceLineInfo::IsChangeLanePath() const {
  return !Lanes().IsOnSegment(); //自车不在当前车道上
}
...
double ReferenceLineInfo::SDistanceToDestination() const {
  ...
  const auto* dest_ptr = path_decision_.Find(FLAGS_destination_obstacle_id); //DEST
  if (!dest_ptr) {
    return res;
  }

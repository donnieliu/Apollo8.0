...
void EgoInfo::Clear() {
  ...
  front_clear_distance_ = FLAGS_default_front_clear_distance;   //默认300米
}
...
void EgoInfo::CalculateFrontObstacleClearDistance(
  ...
  Box2d ego_front_region(center + unit_vec_heading * kDistanceThreshold / 2.0,  //检测区域的bounding_box
                         vehicle_state_.heading(), impact_region_length,
                         param.width() + buffer);

  for (const auto& obstacle : obstacles) {
    if (obstacle->IsVirtual() ||
        !ego_front_region.HasOverlap(obstacle->PerceptionBoundingBox())) {   //虚拟障碍物，或者是障碍物的bounding_box再检测区域外边
      continue;
    }

    double dist = ego_box_.center().DistanceTo(                       //找出一个距离本车最近的障碍物
                      obstacle->PerceptionBoundingBox().center()) -
                  ego_box_.diagonal() / 2.0;                          //减去对角线的一半，可能是因为不知道前车的角度，那么这个时候就按照对角线去算

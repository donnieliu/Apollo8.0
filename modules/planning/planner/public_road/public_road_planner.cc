...
//ScenarioManager会实例化一个全局的scenario_manager_来进行场景管理在PublicRoadPlanner初始化时会调用配置文件里的参数来建立对象。
Status PublicRoadPlanner::Init(const PlanningConfig& config) {
  config_ = config;
  scenario_manager_.Init(config);  //初始化时注册了所有场景，并随手创建了一个默认的场景
  return Status::OK();
}
...
Status PublicRoadPlanner::Plan(const TrajectoryPoint& planning_start_point,
  ...
  if (result == scenario::Scenario::STATUS_DONE) {  //场景处理成功
    // only updates scenario manager when previous scenario's status is
    // STATUS_DONE
    scenario_manager_.Update(planning_start_point, *frame);
  } else if (result == scenario::Scenario::STATUS_UNKNOWN) {  //场景处理失败

...
ReferenceLineProvider::ReferenceLineProvider(
  ...
  if (!FLAGS_use_navigation_mode) {
    pnc_map_ = std::make_unique<hdmap::PncMap>(base_map);  //非导航模式创建pncmap
  ...
  ACHECK(cyber::common::GetProtoFromFile(FLAGS_smoother_config_filename,   //smoother_config_filename=/apollo/modules/planning/conf/discrete_points_smoother_config.pb.txt
                                         &smoother_config_))
      << "Failed to load smoother config file "
      << FLAGS_smoother_config_filename;
  if (smoother_config_.has_qp_spline()) {
    smoother_.reset(new QpSplineReferenceLineSmoother(smoother_config_));  //有三种平滑方法可以选择，配置文件里定义了DiscretePointsReferenceLineSmoother，因此按照protobuf本代码采用离散点平滑。
...
bool ReferenceLineProvider::Start() {  //参考线生成器开始函数
  if (FLAGS_use_navigation_mode) {
    return true;
  }
  if (!is_initialized_) {    //在这个类ReferenceLineProvider初始化的时候会生成平滑用哪个算法，那么会生成对应的平滑类，这过程完成之后就把is_initialized_置为TRUE
    AERROR << "ReferenceLineProvider has NOT been initiated.";
    return false;
  }

  if (FLAGS_enable_reference_line_provider_thread) {  //是否开启参考线生成线程，默认为TRUE
    task_future_ = cyber::Async(&ReferenceLineProvider::GenerateThread, this);  //Cyber异步调用
  }
  return true;
}
...
void ReferenceLineProvider::Stop() {
  is_stop_ = true;
  if (FLAGS_enable_reference_line_provider_thread) {  //默认为TRUE；
    task_future_.get();
  }
}
...
void ReferenceLineProvider::GenerateThread() {
  while (!is_stop_) {   //默认为false，只有显性的调用ReferenceLineProvider::Stop()时才会终止产生线程
    static constexpr int32_t kSleepTime = 50;  // milliseconds
    cyber::SleepFor(std::chrono::milliseconds(kSleepTime));
    const double start_time = Clock::NowInSeconds();
    if (!has_routing_) {  //调用PNCmap的UpdateRoutingResponse函数会更新
      AERROR << "Routing is not ready.";
      continue;
    }
    std::list<ReferenceLine> reference_lines;
    std::list<hdmap::RouteSegments> segments;
    if (!CreateReferenceLine(&reference_lines, &segments)) {  //从pncmap中获取参考线
      is_reference_line_updated_ = false;    //没有获取到参考线
...
bool ReferenceLineProvider::CreateReferenceLine(
  ...
  {
    std::lock_guard<std::mutex> lock(vehicle_state_mutex_);
    vehicle_state = vehicle_state_;   //vehicle_state_的数据是在该类函数UpdateVehicleState中更新。
  }

  routing::RoutingResponse routing;
  {
    std::lock_guard<std::mutex> lock(routing_mutex_);
    routing = routing_;   //routing_的数据是在该类的函数UpdateRoutingResponse中更新。
  }
  bool is_new_routing = false;
  {
    // Update routing in pnc_map
    std::lock_guard<std::mutex> lock(pnc_map_mutex_);
    if (pnc_map_->IsNewRouting(routing)) {   //检查routing是否与PncMap中的现有Routing相同，注意IsNewRouting（）如果和PncMap相同，那么返回false，否则返回TRUE要更新
      is_new_routing = true;   //如果是新的routing，则更新routing信息
   ...
   //提取了refrenceline的一些信息
  if (!CreateRouteSegments(vehicle_state, segments)) { //从routing中根据自车的状态获取segments
    AERROR << "Failed to create reference line from routing";
    return false;
  }
   //对参考线进行平滑处理
  if (is_new_routing || !FLAGS_enable_reference_line_stitching) { //FLAGS_enable_reference_line_stitching（拼接）:默认为true，这个配置就是是否允许参考线的拼接操作
    for (auto iter = segments->begin(); iter != segments->end();) {
      reference_lines->emplace_back(); //注意这个语法的应用，先放入一个默认初始化的对应类型的对象

...
ReferencePoint ReferenceLine::GetNearestReferencePoint(const double s) const {
  const auto& accumulated_s = map_path_.accumulated_s();    //  accumulated_s是每个点的s的累计vector
...
bool ReferenceLine::GetOffsetToMap(const double s, double* l_offset) const {
  ...
  auto ref_point = GetNearestReferencePoint(s);  //从参考线上找一个离s位置最近的point
  if (ref_point.lane_waypoints().empty()) {
    return false;
  }
  *l_offset = ref_point.lane_waypoints().front().l; //这个地方需要熟悉高精地图格式？？
  return true;
}

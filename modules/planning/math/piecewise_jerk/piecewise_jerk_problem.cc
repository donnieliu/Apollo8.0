...
OSQPData* PiecewiseJerkProblem::FormulateProblem() {
  // calculate kernel,参考https://blog.csdn.net/weixin_34945803/article/details/106576629
  std::vector<c_float> P_data;  //P矩阵上面行列索引对应的元素值的数组
  std::vector<c_int> P_indices;  //P矩阵行索引数组
  std::vector<c_int> P_indptr;  // P矩阵列索引数组
  ...
  // calculate offset
  std::vector<c_float> q;  //构建二次规划问题的q矩阵,构建结果储存到输入的指针q里
  CalculateOffset(&q);
...
OSQPSettings* PiecewiseJerkProblem::SolverDefaultSettings() {
  ...
  settings->verbose = FLAGS_enable_osqp_debug;  //false

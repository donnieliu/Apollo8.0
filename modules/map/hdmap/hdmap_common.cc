...
void LaneInfo::Init() {
  ...
  for (size_t i = 0; i + 1 < points_.size(); ++i) {
    segments_.emplace_back(points_[i], points_[i + 1]);  //segments_为LineSegment2d类型
    accumulated_s_.push_back(s);
    unit_directions_.push_back(segments_.back().unit_direction()); //每两个点为一个LineSegment2d，这两个点计算的方向向量为unit_direction
  ...
  unit_directions_.push_back(unit_directions_.back());  //可能是为了保持segments_和点的数量一样,把segments_的方向作为点的方向
  ...
  if (lane_.has_type()) {
    if (lane_.type() == Lane::CITY_DRIVING) {
      for (const auto &p : sampled_left_width_) {
        if (p.second < FLAGS_half_vehicle_width) {  //1.05
          AERROR
  ...
  sampled_left_road_width_.clear(); //不知道这个是什么东西
...
bool LaneInfo::GetProjection(const Vec2d &point, double *accumulate_s,
  ...
  const auto prod = nearest_seg.ProductOntoUnit(point); //叉乘(垂直距离)
  const auto proj = nearest_seg.ProjectOntoUnit(point); //点乘（水平距离）
  if (min_index == 0) {
    *accumulate_s = std::min(proj, nearest_seg.length());
    if (proj < 0) {
      *lateral = prod;
    } else {
      *lateral = (prod > 0.0 ? 1 : -1) * min_dist; //为什么不直接把prod直接给lateral？？
    }

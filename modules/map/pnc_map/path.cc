...
LaneBoundaryType::Type LeftBoundaryType(const LaneWaypoint& waypoint) {
  ...
  for (const auto& type :
       waypoint.lane->lane().left_boundary().boundary_type()) { //注意这里的left_boundary和boundary_type都是proto中的结构
    if (type.s() <= waypoint.s) { //注意这里的s是Offset relative to the starting point of boundary，如果没写入值得话，猜测应该就是lane的起始点
      if (type.types_size() > 0) {
        return type.types(0); //应该是本车道的左侧
      } else {
        return LaneBoundaryType::UNKNOWN;
      }
    }
